...
public class Client {
    ...
    Client(String ip, String videoName, String fileSave, String serverName, String dir,boolean zero_mode,int row) {
        this.ip = ip;
    ...
    void connectToServer() {
        ...
        System.out.println("192.168.56.103: Server");
        ...
        try {
            ...
            if (size == -1) {
                ...
                if (zero_mode) {
                    ...
                    new ZeroCopyDownload(new InetSocketAddress(ip, 50003), size, filePath, newFileLocal, buffer,frame,row);
                    ...
                    frame.setProgessBar(startt,0,true,row);
                    for (int i = 1; i <= NUM_THREADS; i++) {
                        threads[i - 1] = new DownloadThread(i, ip, 50003, bufferSize,
                                size, filePath,newFileLocal,buffer,frame,startt,row);
...
class DownloadThread extends Thread {
    ...
    public DownloadThread(int i, String ip, int port, long bufferSize, long size, String filePath, String path_new,
                          int buffer,MyFrame frame,long startd,int row) throws IOException {
        this.startd = startd;
        this.frame = frame;
        this.row = row;
        thread_id = i;
...
class ZeroCopyDownload {
    ...
    public ZeroCopyDownload(SocketAddress socketAddress, long size, String filePath, String newFile, int buffer,MyFrame frame,int row)
            throws IOException {
        this.size = size;
        this.frame = frame;
        this.row = row;
